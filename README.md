...
│   ├── docx/                        # Word 文档处理
│   ├── drawio/                      # draw.io 图表生成
│   ├── dubbo-remote-service/        # Dubbo 远程服务接口生成
│   ├── entity-field-change/         # 实体字段同步修改
│   ├── grill-me/                    # 设计方案压力测试
│   ├── pdf/                         # PDF 处理
...
根据业务模块中的本地 Service 方法生成对应的 Dubbo 远程服务接口，供微服务模块间跨模块 RPC 调用。

- 根据模块映射自动确定 API 模块、接口包路径和实现包路径
- 自动生成 Remote 接口、RemoteVo 响应对象、Dubbo 服务实现类三件套
- 已有接口时自动追加方法，无需重复创建
- 支持 `@TenantIgnore` 跨租户查询、基本类型返回等特殊情况
- 使用 `BeanUtil.copyProperties` 进行对象转换，构造器注入依赖

**许可**: [MIT](skills/dubbo-remote-service/LICENSE) — Elon Z (2026)

---

### entity-field-change — 实体字段同步修改

修改或删除项目实体类中的字段定义，并自动同步所有关联文件，适用于基于 RuoYi-Cloud-Plus 框架的微服务项目。

- 同步更新 Entity、BO、VO、RemoteVo 四类文件
- 自动处理校验注解变更（`@NotBlank` ↔ `@NotNull`）
- 支持字段类型变更和字段删除两种场景
- 内置类型变更检查清单和异常提醒机制
- 自动搜索引用并清理 getter/setter 调用

**许可**: [MIT](skills/entity-field-change/LICENSE) — Elon Z (2026)
...
#### 实际运行效果图
需要审批
<img width="213" height="145" alt="image" src="https://github.com/user-attachments/assets/4efbf05a-76ac-4cae-b82b-3b338b4cf93d" />



任务完成
<img width="213" height="145" alt="image" src="https://github.com/user-attachments/assets/4f809385-24fc-4f2c-a9db-e3c23d85ba52" />
...
| dubbo-remote-service | MIT | Elon Z (2026) |
| entity-field-change | MIT | Elon Z (2026) |
